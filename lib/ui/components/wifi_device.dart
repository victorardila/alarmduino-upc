...
class WifiDevice extends StatefulWidget {
  const WifiDevice({super.key});

  @override
  State<WifiDevice> createState() => _WifiDeviceState();
}
...
class _WifiDeviceState extends State<WifiDevice> {
  ...
  Future<void> _connectToWifi(String ssid, String password) async {
    // Verifica si ya estás conectado a una red
    bool isConnected = await WiFiForIoTPlugin.isConnected();
    if (isConnected) {
      print('Ya estás conectado a una red WiFi.');
      return;
    }

    // Conecta a la red WiFi con el SSID y contraseña proporcionados
    WiFiForIoTPlugin.connect(ssid,
            password: password, security: NetworkSecurity.WPA)
        .then((value) {
      print('Conexión exitosa a la red WiFi: $ssid');
    }).catchError((error) {
      print('Error al conectar a la red WiFi: $error');
    });
  }

  // Función para desconectar de una red WiFi
  Future<void> _disconnectFromWifi() async {
    // Verifica si estás conectado a una red
    bool isConnected = await WiFiForIoTPlugin.isConnected();
    if (!isConnected) {
      print('No estás conectado a una red WiFi.');
      return;
    }

    // Desconecta de la red WiFi actual
    WiFiForIoTPlugin.disconnect().then((value) {
      print('Desconexión exitosa de la red WiFi.');
    }).catchError((error) {
      print('Error al desconectar de la red WiFi: $error');
    });
  }
